...
function initMap() {
    ...
    fetch(`https://backend-production-47d1.up.railway.app/get-sports-fields`)
            ...
            for (const field of fields) {
                ...
                try {
                    const response = await fetch(
                        `https://backend-production-47d1.up.railway.app/get-coordinates?address=${encodeURIComponent(address)}`
                    );
                ...
                if (confirmReservation) {
                    ...
                    try {
                        ...
                        if (result.success) {
                            ...
                            try {
                                const paymentResponse = await fetch(`https://backend-production-47d1.up.railway.app/create-checkout-session`, {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({
...
function fetchReservations(id_teren) {
    ...
    fetch(`https://backend-production-47d1.up.railway.app/get-field-reservations/${id_teren}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
